...
public class MainServer {
            ...
            if ("ENVOYER".equalsIgnoreCase(command)) {
                ...
                for (SubServerInfo subServer : SUB_SERVERS) {
                    try (Socket subSocket = new Socket(subServer.host, subServer.port);
                        ...
                        System.out.println("Sous-serveur " + subServer.host + " a répondu avec " + fileCount + " fichiers."); // Notification
    ...
    private static void deleteFileFromActiveSubServers(String fileName, List<SubServerInfo> activeSubServers, DataOutputStream dos) throws IOException {
        ...
        for (SubServerInfo subServer : activeSubServers) {
            try (Socket subSocket = new Socket(subServer.host, subServer.port);
                ...
                if (!"OK".equalsIgnoreCase(response)) {
                    System.err.println("Erreur lors de la suppression du fichier " + fileName + " sur " + subServer.host);
                    success = false;
                }
            } catch (IOException e) {
                success = false;
    ...
    private static void deleteFileFromAllSubServers(String fileName, DataOutputStream dos) throws IOException {
        ...
        for (SubServerInfo subServer : SUB_SERVERS) {
            try (Socket subSocket = new Socket(subServer.host, subServer.port);
                ...
                // Envoyer la commande de suppression pour tous les fragments associés au fichier.
                ...
                String response = subDis.readUTF();
                if (!"OK".equalsIgnoreCase(response)) {
                    System.err.println("Erreur lors de la suppression des fragments de " + fileName + " sur " + subServer.host);
                    success = false;
                }
            } catch (IOException e) {
                success = false;
    ...
    private static void distributeAndReplicateFile(DataInputStream dis, String fileName, long fileSize) throws IOException {
        ...
        for (int i = 0; i < SUB_SERVERS.size(); i++) {
            ...
            try (Socket subServerSocket = new Socket(SUB_SERVERS.get(i).host, SUB_SERVERS.get(i).port);
                ...
                System.out.println("Fichier fragmenté envoyé à " + SUB_SERVERS.get(i).host + ":" + SUB_SERVERS.get(i).port); // Notification
            }
    ...
    private static void replicateToOtherSubServers(String fileName, int sourceSubServerIndex) {
        for (int j = 0; j < SUB_SERVERS.size(); j++) {
            if (j != sourceSubServerIndex) {
                try (Socket sourceSocket = new Socket(SUB_SERVERS.get(sourceSubServerIndex).host, SUB_SERVERS.get(sourceSubServerIndex).port);
                    ...
                    System.out.println("Fragment " + fileName + " répliqué à " + SUB_SERVERS.get(j).host + ":" + SUB_SERVERS.get(j).port); // Notification
                } catch (IOException e) {
            ...
            for (int i = 0; i < SUB_SERVERS.size(); i++) {
                ...
                for (int attempt = 0; attempt < 3; attempt++) {
                    ...
                    try (Socket subServerSocket = new Socket(subServer.host, subServer.port);
                        ...
                        System.err.println("Erreur avec le sous-serveur " + subServer.host + " pour le fragment " + fileName + ".part" + i);
                    }
                }
    
                if (!fragmentRetrieved) {
                    throw new IOException("Impossible de récupérer le fragment " + fileName + ".part" + i + " après plusieurs tentatives.");
                }
            }
        }
